...
def _attach_manifest(
    ...
    *,
    ...
    """
    Phase 1 policy:
      - TOPO / SSURGO / WEATHER are expected to be 1-row-per-ffy manifests or summaries.
      - We repeat them to match plot-level exp rows.
      - If manifest is missing, keep going (pipeline shouldn't block).
    """
    ...
    if len(manifest) == 1:
        ...
        # ensure status col exists if requested
        if status_col and status_col not in out.columns:
            out[status_col] = "OK"
        return out

    # If later you decide to precompute plot-level features already, allow concat-by-rowcount.
    ...
    # Otherwise do NOT try to merge in Phase 1 (that's Phase 2's job).
    logger.warning(
        "[%s] %s table has %d rows (expected 1 or %d). Skipping %s attach in Phase 1.",
        ffy_id, name, len(manifest), len(df), name
    )
    if status_col and status_col not in df.columns:
...
def _build_one(
    *,
    ...
    # Attach manifests (repeat to plot rows)
    df = _attach_manifest(df, topo, logger, ffy_id, name="TOPO", status_col="topo_status", missing_status="MISSING_TOPO")
    df = _attach_manifest(df, ssurgo, logger, ffy_id, name="SSURGO", status_col="ssurgo_status", missing_status="MISSING_SSURGO")

    # Weather status is optional; keep if you like sanity flags
    df = _attach_manifest(df, weather, logger, ffy_id, name="WEATHER", status_col="weather_status", missing_status="MISSING_WEATHER")
...
def run(*, overwrite: bool = False) -> None:
    ...
    Phase 1 `model_ready` parquet = exp_table + repeated 1-row manifests:
      - exp_table (plot-level)
      - enriched_weather (1 row/ffy -> repeated)
      - enriched_ssurgo  (1 row/ffy manifest -> repeated)
      - enriched_topo    (1 row/ffy manifest -> repeated)

    This file should NOT do expensive spatial overlay or per-subpolygon extraction.
    """
    logger = get_logger()
...
def run_one_field(ffy_id: str, *, overwrite: bool = False) -> None:
    """
    Debug runner: rebuild model_ready for a single ffy_id.
    """
    logger = get_logger()
    paths = ProjectPaths(Path("."))
    ensure_dir(paths.parquet_model_ready_dir)
